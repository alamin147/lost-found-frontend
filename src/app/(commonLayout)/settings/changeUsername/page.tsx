...
const Page = () => {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      const res: any = await changeUsername({ username });
      console.log(res);
      ...
      if (res?.data?.statusCode == 200) {
        Modals({
          message: `${res?.data?.message}. Your new username is ${username}. Now please login again!`,
          status: true,
        });
        reset();
      ...
      Modals({ message: "Failed to change username.", status: false });
    }
  };
